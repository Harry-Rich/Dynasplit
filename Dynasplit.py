...
class BeadGroup(object):
    ...
    def positions(self):
        return np.array(
            [g.center_of_mass(unwrap=True) for g in self._groups], dtype=np.float32
        )
    ...
    def ind_mass_check(self, indices, masses):
        if indices is None:
            try:
                ...
                raise ValueError(
                    "Molecules not defined as fragments; please provide `indices`."
                ) from e
        ...
        if masses is None:
            try:
                ...
                raise ValueError(
                    "Atom masses not defined in the universe; please provide `masses`."
                ) from e

        return np.array(indices), np.array(masses)

    def write(
        self, to_write="both", trans_file="translation.dcd", rot_file="rotation.dcd"
    ):
