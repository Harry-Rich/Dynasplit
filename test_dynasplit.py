...
def u():
    return mda.Universe(
        "./test_traj/290k_end.data", "./test_traj/sampled_100_frames.dcd"
    )  # Tiny test files
...
def test_dynasplit_com_consistency(u):
    ...
    fast = ds.Dynasplit(u)
    splitter.calc_com(
        slower=True, atom_types="type 1"
    )  # Uses fragments so typing doesn't really matter
    fast.calc_com()
    assert np.all(np.isclose(fast.com_array, splitter.com_array, atol=1e-6)), (
        "Center of mass calculations differ beyond tolerance."
    )
